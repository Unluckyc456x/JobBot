...
def init_supabase():
    res = supabase.table("cards").select("card_id", count="exact").execute()
    if res.count == 0:
        rows = []
...
def is_user_blocked(user_id):
    ...
    if res.data:
        row = res.data[0]
        if row["is_frozen"] or row["is_banned"]:
            return True
    return False

def freeze_user(user_id, reason):
    now = datetime.now(timezone.utc).isoformat()
    supabase.table("users").update({"is_frozen": 1}).eq("user_id", user_id).execute()
...
def can_roll(user_id):
    res = supabase.table("users").select("last_roll").eq("user_id", user_id).execute()
    if not res.data or not res.data[0]["last_roll"]:
        return True, None
    last = datetime.fromisoformat(res.data[0]["last_roll"])
...
def give_card_to_user(user_id, card, now):
    user_res = supabase.table("users").select("roll_count, total_cards, total_jobs").eq("user_id", user_id).execute()
    u_data = user_res.data[0]
    
    # Регистрируем карту
    card_res = supabase.table("user_cards").select("count").eq("user_id", user_id).eq("card_id", card["card_id"]).execute()
    if card_res.data:
        new_cnt = card_res.data[0]["count"] + 1
        supabase.table("user_cards").update({"count": new_cnt}).eq("user_id", user_id).eq("card_id", card["card_id"]).execute()
    else:
        supabase.table("user_cards").insert({"user_id": user_id, "card_id": card["card_id"], "count": 1}).execute()

    supabase.table("users").update({
        "roll_count": u_data["roll_count"] + 1,
        "total_cards": u_data["total_cards"] + 1,
        "total_jobs": u_data["total_jobs"] + card["jobs_award"],
...
async def roll_card(message: Message):
    ...
    res = supabase.table("users").select("roll_count").eq("user_id", user_id).execute()
    roll_cnt = res.data[0]["roll_count"]
...
async def render_mycards_page(user_id, message_or_cb, page_idx=0):
    ...
    cards_res = supabase.table("user_cards").select("count, cards(name, series, rarity)").eq("user_id", user_id).execute()
    ...
    if cards_res.data:
        for row in cards_res.data:
            ...
            cnt = row.get("count", 0)
            total_cards += cnt
            if c and c.get("rarity") == rarity:
                filtered_cards.append({"name": c["name"], "series": c["series"], "count": cnt})
    ...
    if not filtered_cards:
        ...
        for r in sorted(filtered_cards, key=lambda x: x["name"]):
            msg_text += f"• <b>{html.escape(r['name'])}</b> ({html.escape(r['series'])}) — <b>{r['count']} шт.</b>\n"
...
async def show_balance(message: Message):
    if await check_antispam(message, bot): return
    res = supabase.table("users").select("total_jobs").eq("user_id", message.from_user.id).execute()
    jobs = res.data[0]["total_jobs"] if res.data else 0
...
async def top_jobs(message: Message):
    if await check_antispam(message, bot): return
    try:
        res = supabase.table("users").select("username, total_jobs").gt("total_jobs", 0).eq("is_banned", 0).order("total_jobs", desc=True).limit(30).execute()
        if not res.data:
...
async def check_user(message: Message):
    ...
    uid = user["user_id"]
    status = "⛔ ЗАБАНЕН" if user["is_banned"] else ("🧊 ЗАМОРОЖЕН" if user["is_frozen"] else "🟢 Активен")
    ...
    top_res = supabase.table("users").select("user_id", count="exact").gt("total_jobs", user["total_jobs"]).eq("is_banned", 0).execute()
    top_pos = (top_res.count or 0) + 1

    msg = (
        f"📋 <b>ИНСПЕКЦИЯ ПОЛЬЗОВАТЕЛЯ</b>\n──────────────────────\n"
        f"👤 Игрок: @{html.escape(user['username'] or 'no_name')} (ID: <code>{uid}</code>)\n"
        f"💰 Баланс: <b>{user['total_jobs']}</b> джобсов | 🏆 Топ: #{top_pos} | 🎲 Роллов: {user['roll_count']}\n"
        f"🧊 Статус: <b>{status}</b>\n"
...
async def logs_user(message: Message):
    ...
    uid = user["user_id"]
    spams = supabase.table("spam_logs").select("timestamp, reason").eq("user_id", uid).order("id", desc=True).limit(5).execute()
    ...
    if not spams.data:
        ...
        for s in spams.data:
            msg += f"• {s['timestamp']} | {html.escape(s['reason'])}\n"
...
async def cmd_freeze(message: Message):
    ...
    user = get_target_user(args[1])
    if user:
        supabase.table("users").update({"is_frozen": 1}).eq("user_id", user["user_id"]).execute()
...
async def cmd_unfreeze(message: Message):
    ...
    user = get_target_user(args[1])
    if user:
        supabase.table("users").update({"is_frozen": 0}).eq("user_id", user["user_id"]).execute()
...
async def cmd_ban(message: Message):
    ...
    user = get_target_user(args[1])
    if user:
        supabase.table("users").update({"is_banned": 1}).eq("user_id", user["user_id"]).execute()
        await message.answer(f"⛔ Пользователь @{html.escape(user['username'] or 'no_name')} забанен и скрыт из ТОПа.", parse_mode=ParseMode.HTML)
...
async def cmd_unban(message: Message):
    ...
    user = get_target_user(args[1])
    if user:
        supabase.table("users").update({"is_banned": 0}).eq("user_id", user["user_id"]).execute()
        await message.answer(f"✅ Пользователь @{html.escape(user['username'] or 'no_name')} разбанен.", parse_mode=ParseMode.HTML)
...
async def cmd_rc(message: Message):
    ...
    user = get_target_user(args[1])
    if user:
        supabase.table("users").update({"last_roll": None}).eq("user_id", user["user_id"]).execute()
...
async def reset_user(message: Message):
    ...
    supabase.table("users").update({"total_cards": 0, "total_jobs": 0, "last_roll": None, "roll_count": 0}).eq("user_id", target_id).execute()
    await message.answer(f"✅ Прогресс пользователя @{html.escape(user['username'] or 'no_name')} (ID: {target_id}) полностью сброшен.", parse_mode=ParseMode.HTML)
...
async def give_jobs(message: Message):
    ...
    new_jobs = user["total_jobs"] + amount
    supabase.table("users").update({"total_jobs": new_jobs}).eq("user_id", user["user_id"]).execute()
    await message.answer(f"✅ Выдано {amount} джобсов пользователю @{html.escape(user['username'] or 'no_name')}.", parse_mode=ParseMode.HTML)
...
async def give_card(message: Message):
    ...
    card = None
    if query.isdigit():
        res = supabase.table("cards").select("*").eq("card_id", int(query)).execute()
        card = res.data[0] if res.data else None
    if not card:
        res = supabase.table("cards").select("*").ilike("name", f"%{query}%").execute()
        card = res.data[0] if res.data else None
    ...
    caption = (
        f"🃏 <b>Админ-разработчик бота Джоб лично выдал карту</b> «{html.escape(card['name'])} ({html.escape(card['series'])})» пользователю @{html.escape(user['username'] or 'no_name')} 🃏\n"
        f"✨ Редкость: {RARITY_RU[card['rarity']]} {RARITY_EMOJI[card['rarity']]} ✨\n"
        f"💰 Джобсы: +{card['jobs_award']} 💰\n"
        f"<i>«{html.escape(card['quote'])}»</i>"
    )
...
async def cb_unfreeze(cb: CallbackQuery):
    ...
    supabase.table("users").update({"is_frozen": 0}).eq("user_id", uid).execute()
...
async def cb_ban(cb: CallbackQuery):
    if cb.from_user.id != ADMIN_ID: return
    uid = int(cb.data.split(":")[1])
    supabase.table("users").update({"is_banned": 1}).eq("user_id", uid).execute()
